...
export function HostTable({ count = 0, rows = [], page = 0, rowsPerPage = 0 }: HostTableTableProps): React.JSX.Element {
  ...
  const [selectHost, setSelectHost] = useState<Host | undefined>(undefined);
        ...
        <Table sx={{ minWidth: '800px' }}>
          ...
          <TableBody>
            {rows.map((row) => {
              ...
              let circleWeight: 'fill' | undefined = undefined; // 声明 circleWeight 的类型
              let circleColor = '';
              let tooltipTitle = '';
              if (row.online) {
                circleWeight = 'fill';
